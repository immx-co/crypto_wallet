...
@db_session
def create_transaction(sender: User,
    ...
    """
    wallet_of_sender = bit.Key(sender.wallet.private_key) if not testnet else \
        bit.PrivateKeyTestnet(sender.wallet.private_key)
    print(f'sender balance: {wallet_of_sender.get_balance()}')
...
def update_wallet_balance(wallet: pydantic_models.Wallet):
    ...
    bit_wallet = bit.PrivateKeyTestnet(wallet.private_key) if testnet else bit.Key(wallet.private_key)
...
def get_user_transactions(user_id):
    outgoing_transaction_list = [get_transaction_info(trans) for trans in Transaction.select(lambda item: int(item.sender) == user_id)]
    incoming_transaction_list = []
    user_wallet_address = User[user_id].wallet.address
    for trans in Transaction.select():
        if trans.receiver_address == user_wallet_address:
            incoming_transaction_list.append(get_transaction_info(trans))

    return outgoing_transaction_list, incoming_transaction_list
